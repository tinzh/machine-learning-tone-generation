...
def histogram(f, p):
    ...
    f = lambda x,I4,If: np.exp(-(x-I4)**2/(2*If**2))/np.sqrt(2*np.pi)/If

    fI4 = lambda x: f(x, I4, If)
    fI4xs = fI4(xs)
  
    SSR = np.linalg.norm(f(binCentre,I4,If)-hist)**2
    ...
    histBelow = np.select([hist < fI4(binCentre)], [hist], fI4(binCentre))
    histAbove = np.abs(fI4(binCentre)-hist)
    histAbove1 = np.select([hist < fI4(binCentre)], [histAbove], 0)
    histAbove2 = np.select([hist >= fI4(binCentre)], [histAbove], 0)
    ...
    ax.plot(xs, fI4xs, color=white, linewidth=2)
...
def contour(f, p) :
    ...
    Ifmax = 35
    Ifmin = 2
    I4s = np.arange(I4min,I4max+.5,.5)
    Ifs = np.arange(Ifmin,Ifmax+.5,.5)
    M, S = np.meshgrid(I4s, Ifs)
    Z = ( (M - (I4max+I4min)/2)**2 + ((I4max-I4min)/Ifmax / 1.333)**2*(S - Ifmax)**2 )**0.5 /( (I4min - (I4max+I4min)/2)**2 + ((I4max-I4min)/Ifmax / 1.333)**2*(0 - Ifmax)**2 )**0.5

    fig,ax = plt.subplots(figsize=(8, 6), dpi= 80, facecolor=blue1)

    ax.set_xlim([I4min,I4max])
    ax.set_ylim([Ifmin,Ifmax])
    ax.set_aspect(1)

    ax.set_xlabel("$\mu / \mathrm{cm} $", fontsize=14)
    ax.set_ylabel("$\sigma / \mathrm{cm} $", fontsize=14)
    
    im = ax.imshow(Z, vmin=0, vmax=1, extent=[I4min, I4max, Ifmax, Ifmin], cmap=blueMap)


    z = ((f(binCentre.reshape(binCentre.size,1,1),I4s.reshape(1,I4s.size,1),Ifs.reshape(1,1,Ifs.size)) - hist.reshape(hist.size,1,1))**2).sum(axis=0)
    ax.contour(np.outer(I4s, np.ones_like(Ifs)),np.outer(np.ones_like(I4s), Ifs),z,20, colors=[green], linewidths=1.5)
